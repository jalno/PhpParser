...
use packages\PhpParser\PrettyPrinter;
use packages\PhpParser\NodeTraverser;
use packages\PhpParser\NodeVisitor\NameResolver;
...
class NamespaceConverter extends\packages\PhpParser\NodeVisitorAbstract
{
    public function leaveNode(Node $node) {
...
class NodeVisitor_NamespaceConverter extends\packages\PhpParserNodeVisitorAbstract
{
    public function leaveNode(Node $node) {
        if ($node instanceof Node\Name) {
            return new Node\Name(str_replace('\\', '_', $node->toString()));
